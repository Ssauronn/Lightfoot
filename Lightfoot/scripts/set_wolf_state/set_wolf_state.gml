...
function set_wolf_state() {
	// All actions are available if the player is not next to a bonfire.
	...
	// If the player is within range of a bonfire, then give up and return to patrolling.
		...
		// If the player is not in an authorized action, set it to an authorized action
		// and reset relevant variables before continuing.
		if (wolfCurrentAction != wolfActionState.returntopatrol) && (wolfCurrentAction != wolfActionState.bigpatrol) && (wolfCurrentAction != wolfActionState.idle) {
			canSeePlayer = false;
		...
		// Only actions involving patrolling are available to Wolves when the
		// player is next to a bonfire.
		switch wolfCurrentAction {
			case wolfActionState.idle:
				wolf_idle();
				break;
